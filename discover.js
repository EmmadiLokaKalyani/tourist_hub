...
function fetching(pageNum) {
    fetch('./dest.json')  // Assuming dest.json is in the same directory as your HTML file
        .then(function(res) {
            return res.json();
        })
        .then(function(datas) {
            let startIndex = (pageNum - 1) * 9;
            let endIndex = startIndex + 9;
            let pageData = datas.slice(startIndex, endIndex);
            
            let totalBtn = Math.ceil(datas.length / 9); // Calculate total number of pages
